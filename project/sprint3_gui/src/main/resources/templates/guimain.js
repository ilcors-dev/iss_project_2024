   ...
   socket = new WebSocket(addr);
    ...
    socket.onmessage = function(event) {
        ...
        if (wasteStorageMatch) {
            document.getElementById('waste-storage').textContent = wasteStorageMatch[1];
        }
        if (incineratorStatusMatch) {
            document.getElementById('incinerator-status').textContent = incineratorStatusMatch[1];
        }
        if (robotLocationMatch) {
            document.getElementById('robot-location').textContent = robotLocationMatch[1];
        }
        if (ashStorageMatch) {
